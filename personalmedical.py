...
def personalmedical_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(personalmedical,tablename)
    print(fexec.get_result())    
    
def personalmedical(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT noreg, name, value, date, idform FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(noreg, name, value, date, \
                    idform) VALUES(%s, %s, %s, %s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(personalmedical,'personalmedical')
    print(fexec.get_result())

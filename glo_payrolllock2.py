...
def glo_payrolllock2_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_payrolllock2,tablename)
    print(fexec.get_result())    
    
def glo_payrolllock2(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT tipe, bulan, tahun, approver, appdate, appip FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(tipe, bulan, tahun, \
                    approver, appdate, appip) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_payrolllock2,'glo_payrolllock2')
    print(fexec.get_result())

...
def kurs_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kurs,tablename)
...
def kurs(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT kurs, curr FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(kurs, curr) \
                    VALUES(%s, %s)",records)
    conn2.commit()

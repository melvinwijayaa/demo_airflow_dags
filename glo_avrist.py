...
def glo_avrist_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_avrist,tablename)
    print(fexec.get_result())    
    
def glo_avrist(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, lob, type, vip, plan1, plan2, stat, username, \
                tglsek, plan3 FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, lob, type, vip, \
                    plan1, plan2, stat, username, tglsek, plan3) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_avrist,'glo_avrist')
    print(fexec.get_result())

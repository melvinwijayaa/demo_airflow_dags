...
def glo_expensesrule_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_expensesrule,tablename)
    print(fexec.get_result())    
    
def glo_expensesrule(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT kodenama, nilai FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(kodenama, nilai) VALUES(%s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_expensesrule,'glo_expensesrule')
    print(fexec.get_result())

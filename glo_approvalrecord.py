...
def glo_approvalrecord_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_approvalrecord,tablename)
    print(fexec.get_result())    
    
def glo_approvalrecord(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, uname, tgl, ip, tipe, status, \
                pesan, idnya, createdby, createddate, createdip FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, stat, uname, \
                    tgl, ip, tipe, status, pesan, idnya, createdby, createddate, createdip) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_approvalrecord,'glo_approvalrecord')
    print(fexec.get_result())

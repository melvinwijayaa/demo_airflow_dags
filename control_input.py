...
def control_input_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(control_input,tablename)
    print(fexec.get_result())    
    
def control_input(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, uname, tgl, ip, action, value_input, message, status FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO jtiiasset." +tablename+ "(id, stat, uname, tgl, ip, action, value_input, message, status) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(control_input,'control_input')
    print(fexec.get_result())
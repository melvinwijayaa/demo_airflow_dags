...
def contract_termination_request_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(contract_termination_request,tablename)
    print(fexec.get_result())    
    
def contract_termination_request(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, contract_id, requester, request_date, reviewer, \
                  review_date, status, termination_date, remark FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, contract_id, requester, \
                    request_date, reviewer, review_date, status, termination_date, remark) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(contract_termination_request,'contract_termination_request')
    print(fexec.get_result())

...
def glo_pperiod_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pperiod,tablename)
    print(fexec.get_result())    
    
def glo_pperiod(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT month, firstdate, firstmonth, enddate, endmonth, lob, tipe FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(month, firstdate, firstmonth, \
                    enddate, endmonth, lob, tipe) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pperiod,'glo_pperiod')
    print(fexec.get_result())

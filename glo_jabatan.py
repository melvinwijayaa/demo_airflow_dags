...
def glo_jabatan_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_jabatan,tablename)
    print(fexec.get_result())    
    
def glo_jabatan(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT name, sink FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(name, sink) \
        VALUES(%s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_jabatan,'glo_jabatan')
    print(fexec.get_result())

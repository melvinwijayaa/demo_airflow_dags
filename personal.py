...
def personal_function(tablename):
    fexec = lithops.FunctionExecutor(config=config)
    fexec.call_async(personal,tablename)
    print(fexec.get_result())    
    
def personal(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT idpersonal, badge, nama, jnkel, tgllhr, tptlhr, alamat, telepon, nohp, email, agama, statnkh, status, foto, bankname, bankbranch, bankaccount, accountname, nojamsos, npwp, passid, idabsen, xtglkerja, xquitkerja, wn, wp, ktp FROM "+ tablename)
    records = cur1.fetchall()
    #conn1.commit() -- no need to commit

    #Delete data --change here
    cur2 = conn2.cursor()
    cur2.execute("DELETE FROM livejtiipdbms." +tablename)
    conn2.commit()
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(idpersonal, badge, nama, jnkel, tgllhr, tptlhr, alamat, telepon, nohp, email, agama, statnkh, status, foto, bankname, bankbranch, bankaccount, accountname, nojamsos, npwp, passid, idabsen, xtglkerja, xquitkerja, wn, wp, ktp) \
...
if __name__ == '__main__':
    ...
    fexec.call_async(personal,'personal')
    print(fexec.get_result())
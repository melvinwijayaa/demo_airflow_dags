...
def global_taxpersen_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(global_taxpersen,tablename)
    print(fexec.get_result())    
    
def global_taxpersen(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, username, tgl, ip, stat, effective, bawah, atas, \
                persen, npwp FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, username, tgl, ip, \
                    stat, effective, bawah, atas, persen, npwp) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(global_taxpersen,'global_taxpersen')
    print(fexec.get_result())

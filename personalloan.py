...
def personalloan_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(personalloan,tablename)
    print(fexec.get_result())    
    
def personalloan(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT noreg, loantotal, loanbayar, loanbulan, period, tahun, \
                date, idloan, loanbunga, bayar FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(noreg, loantotal, loanbayar, \
                    loanbulan, period, tahun, date, idloan, loanbunga, bayar) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(personalloan,'personalloan')
    print(fexec.get_result())

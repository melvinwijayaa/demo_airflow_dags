...
def sec_logmdl_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(sec_logmdl,tablename)
    print(fexec.get_result())    
    
def sec_logmdl(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, uname, tgl, ip, createddate, createdby, nfile, status FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO jtiiasset." +tablename+ "(id, stat, uname, tgl, ip, createddate, createdby, nfile, status) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(sec_logmdl,'sec_logmdl')
    print(fexec.get_result())
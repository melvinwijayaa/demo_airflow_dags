...
def glo_dailyperiod_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_dailyperiod,tablename)
    print(fexec.get_result())    
    
def glo_dailyperiod(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, tglawal, blnawal, tglakhir, \
                blnakhir, lob FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, tglawal, blnawal, \
                    tglakhir, blnakhir, lob) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_dailyperiod,'glo_dailyperiod')
    print(fexec.get_result())

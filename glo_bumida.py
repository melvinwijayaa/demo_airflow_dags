...
def glo_bumida_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_bumida,tablename)
    print(fexec.get_result())    
    
def glo_bumida(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, lob, type, value, stat, \
                username, tglsek FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, lob, type, value, \
                    stat, username, tglsek) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_bumida,'glo_bumida')
    print(fexec.get_result())

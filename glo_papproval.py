...
def glo_papproval_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_papproval,tablename)
    print(fexec.get_result())    
    
def glo_papproval(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT tipe FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(tipe) VALUES(%s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_papproval,'glo_papproval')
    print(fexec.get_result())

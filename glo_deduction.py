...
def glo_deduction_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_deduction,tablename)
    print(fexec.get_result())    
    
def glo_deduction(tablename):
    ...
    cur1.execute("SELECT id, name, value, rumus, lob, department, give, noreg, used, period, \
                year, taxed, tipe, dayrule, report FROM "+ tablename)
    ...
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, name, value, rumus, \
                    lob, department, give, noreg, used, period, \
                    year, taxed, tipe, dayrule, report ) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_deduction,'glo_deduction')
    print(fexec.get_result())

...
def global_ptkp_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(global_ptkp,tablename)
    print(fexec.get_result())    
    
def global_ptkp(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, username, tgl, ip, stat, effective, nama, formula FROM "+ tablename)
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(global_ptkp,'global_ptkp')
    print(fexec.get_result())

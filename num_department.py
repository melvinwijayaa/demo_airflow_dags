...
def num_department_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(num_department,tablename)
    print(fexec.get_result())    
    
def num_department(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, createdby, createddate, createdip, updatedby, updateddate, \
                updatedip, kode, department, kode2, isIBM FROM "+ tablename)
    ...
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, stat, createdby, createddate, \
                    createdip, updatedby, updateddate, updatedip, kode, department, kode2, isibm) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(num_department,'num_department')
    print(fexec.get_result())

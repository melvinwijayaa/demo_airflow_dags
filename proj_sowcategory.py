...
def proj_sowcategory_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_sowcategory,tablename)
    print(fexec.get_result())    
    
def proj_sowcategory(tablename):
    ...
    cur1.execute("SELECT id, stat, createdby, createddate, createdip, updatedby, updateddate, updatedip, \
                 category, requirement FROM "+ tablename)
    ...
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, stat, createdby, createddate, \
                createdip, updatedby, updateddate, updatedip, category, requirement) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_sowcategory,'proj_sowcategory')
    print(fexec.get_result())

...
def proj_sowlayout_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_sowlayout,tablename)
    print(fexec.get_result())    
    
def proj_sowlayout(tablename):
    ...
    cur1.execute("SELECT id, stat, createdby, createddate, createdip, updatedby, updateddate, updatedip, \
                 idcat, layout, first, efdate FROM "+ tablename)
    ...
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, stat, createdby, createddate, \
                createdip, updatedby, updateddate, updatedip, idcat, layout, first, efdate) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_sowlayout,'proj_sowlayout')
    print(fexec.get_result())

...
def proj_departmentlist_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_departmentlist,tablename)
    print(fexec.get_result())    
    
def proj_departmentlist(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, idproject, iddept, monthlypercentage, idc, planfte, startperiod, \
                endperiod, fte, status FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO jtiiproms." +tablename+ "(id, idproject, iddept, \
                monthlypercentage, idc, planfte, startperiod, endperiod, fte, status) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(proj_departmentlist,'proj_departmentlist')
    print(fexec.get_result())

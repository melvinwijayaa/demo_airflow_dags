...
def glo_medical_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_medical,tablename)
    print(fexec.get_result())    
    
def glo_medical(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, lob, name, value, rule1, rule2, rule3, give, \
                every, paid, rule4 FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(id, lob, name, value, \
                    rule1, rule2, rule3, give, every, paid, rule4) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_medical,'glo_medical')
    print(fexec.get_result())

...
def kurangpajak_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kurangpajak,tablename)
    print(fexec.get_result())    
    
def kurangpajak(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT noreg, kurang, fix, strdate, fixmonth, tipe FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipayroll." +tablename+ "(noreg, kurang, fix, strdate, \
                    fixmonth, tipe) VALUES(%s, %s, %s, %s, %s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kurangpajak,'kurangpajak')
    print(fexec.get_result())

...
def superior_function(tablename):
    fexec = lithops.FunctionExecutor(config=config)
    fexec.call_async(superior,tablename)
    print(fexec.get_result())    
    
def superior(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, name, noreg, lob FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, name, noreg, lob) \
...
if __name__ == '__main__':
    ...
    fexec.call_async(superior,'superior')
    print(fexec.get_result())